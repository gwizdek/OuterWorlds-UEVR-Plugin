...
std::string OuterWorldsPlugin::get_test_string() {
    return "Generated by Main Plugin";
}
...
void OuterWorldsPlugin::prepare_pointers() {
    ...
    try {
        ...
        API::get()->log_error("[prepare_pointers] GetWorld");
        return;
    }
    ...
    try {
        ...
        API::get()->log_error("[prepare_pointers] GetIndianaPC");
        return;
    }
    ...
    try {
        ...
        API::get()->log_error("[prepare_pointers] GetIndianaPlayerController");
        return;
    }
    if (m_player_controller != nullptr) {
        try {
            ...
            API::get()->log_error("[prepare_pointers] GetIndianaUI");
            return;
        }
        if (m_ui != nullptr) {
            try {
                ...
                API::get()->log_error("[prepare_pointers] GetHUD");
                return;
            }
        }
    }
    ...
    try {
        ...
        API::get()->log_error("[prepare_pointers] Set VR HUD pointers");
        return;
    }
}
...
void OuterWorldsPlugin::handle_vr_view(const UEVR_VRData* vr) {
    try {
        ...
        //if (m_ui_conversation_open_state.has_changed()) {
        //    if (m_ui_conversation_open_state.value == SDK::EWidgetOpenState::Maximized) {
        //        vr->set_aim_method(0);
        //    }
        //    else {
        //        vr->set_aim_method(2);
        ...
        if (m_is_conversation_camera_active.enabled()) {
            ...
            API::UObjectHook::set_disabled(true);
            vr->recenter_view();
            vr->set_mod_value("UI_Distance", "1.600000");
            vr->set_mod_value("UI_Size", "1.100000");
        ...
        //if (m_ui_computer_terminal_open_state.has_changed()) {
        //    API::get()->log_warn("[handle_vr_view] Computer Terminal State Change");
        //    if (m_ui_computer_terminal_open_state.value == SDK::EWidgetOpenState::Maximized) {
        //        API::get()->log_warn("[handle_vr_view] Opening Computer Terminal");
        //        vr->set_aim_method(0);
        //    }
        //    else {
        //        API::get()->log_warn("[handle_vr_view] Closing Computer Terminal");
        //        vr->set_aim_method(2);
        //    }
        //}
        ...
        API::get()->log_error("[handle_vr_view]");
    }
    ...
    try {
        if (m_equipped_weapon.value != nullptr && m_equipped_weapon.value->SkeletalMeshComponent != nullptr && is_vr_hud_valid()) {
            ...
            auto offset_transform = m_equipped_weapon.value->SkeletalMeshComponent->GetSocketTransform(
                SDK::UKismetStringLibrary::Conv_StringToName(L"Barrel_JNT"), SDK::ERelativeTransformSpace::RTS_Component
                //SDK::UKismetStringLibrary::Conv_StringToName(L"Iron_Sight_JNT"), SDK::ERelativeTransformSpace::RTS_Component
            );
            ...
            //auto world_offset_transform = m_equipped_weapon.value->SkeletalMeshComponent->GetSocketTransform(
            //    SDK::UKismetStringLibrary::Conv_StringToName(L"Barrel_JNT"), SDK::ERelativeTransformSpace::
            //    //SDK::UKismetStringLibrary::Conv_StringToName(L"Iron_Sight_JNT"), SDK::ERelativeTransformSpace::RTS_Component
            //);

            //API::get()->log_warn("[fix_weapon_ironsights_offset] transform %f %f %f", offset_transform.Translation.X, offset_transform.Translation.Y, offset_transform.Translation.Z);
            //API::get()->log_warn("[fix_weapon_ironsights_offset] world transform %f %f %f", world_offset_transform.Translation.X, world_offset_transform.Translation.Y, world_offset_transform.Translation.Z);
    ...
    try {
        if (m_player_character != nullptr && m_player_character->IsA(SDK::AIndianaPlayerCharacter_BP_C::StaticClass())) {
            ...
            if (class_ptr != nullptr) {
                API::get()->log_warn("[fix_highlighter] Class found");
                ...
                for (size_t i = 0; i < matching_objects.size(); i++) {
                    if (matching_objects[i]->IsA(SDK::UPlayerHighlightComponent_C::StaticClass())) {
                        API::get()->log_warn("[fix_highlighter] Highlighter found, changing params");
...
void OuterWorldsPlugin::internal_frame() {
    ...
    static const auto NO_OPTIONS = std::format("[TODO] Plugin options");
    ...
    if (ImGui::Begin(UEVR_NAME.c_str())) {
        ImGui::TextColored(ImVec4(1.0f, 1.0f, 1.0f, 1.0f), NO_OPTIONS.c_str());
    }
    ImGui::End();
    return;
    ...
    if (ImGui::Begin(UEVR_NAME.c_str())) {
        ...
        if (m_config != nullptr) {
            ImGui::Checkbox("Use Laser Pointer", &(m_config->m_cfg_option_use_laser_pointer));
        ...
        if (m_ui_option_show_debug_view) {
            ...
            static int trace_type_query{ 0 };
            ImGui::SliderInt("Trace Type Query", &trace_type_query, 0, 31);
            if (ImGui::Button("Save Trace Type Query")) {
                if (is_vr_hud_valid()) {
                    m_vr_hud->set_trace_type_query(trace_type_query);
                };
            };
            ...
            //if (m_player_controller != nullptr) {
            //    
            //    ImGui::InputFloat("Hit distance", &(m_player_controller->HitResultTraceDistance));
            //}
